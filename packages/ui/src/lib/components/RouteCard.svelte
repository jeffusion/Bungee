...
<div class="card bg-base-100 shadow-md hover:shadow-lg transition-shadow">
  <div class="card-body flex flex-col">
        ...
        <div class="flex gap-2 mt-2 flex-wrap">
          ...
          {#if route.transformer}
            <span class="badge badge-sm badge-info">
              Transformer: {typeof route.transformer === 'string' ? route.transformer : 'Custom'}
            </span>
          {/if}
    ...
    <!-- Upstreams Summary -->
    <div class="mt-4">
      <p class="text-sm font-semibold mb-2">Upstreams:</p>
      <div class="space-y-2">
        {#each route.upstreams as upstream, index}
          <div class="flex items-center gap-2 text-sm">
            ...
            {#if upstream.weight}
              <span class="badge badge-xs">W: {upstream.weight}</span>
            {/if}
